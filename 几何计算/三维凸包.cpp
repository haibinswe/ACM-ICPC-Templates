...
struct Point
{
    ...
    Point operator * (const Point &p) //叉乘
    {
        return Point(y * p.z - z * p.y, z * p.x - x * p.z, x * p.y - y * p.x);
    }
    double operator ^ (const Point &p) //点乘
    {
...
struct ConvexHull3D
{
    ...
    double dblcmp(Point &p, Surface &f)  //正:点在面同向
    {
        Point m = P[f.b] - P[f.a];
        Point n = P[f.c] - P[f.a];
        Point t = p - P[f.a];
    ...
    int GetTriangle()   //表面三角形个数
    {
        return num;
    }
    int GetPolygon()   //表面多边形个数
    {
        int res = 0, flag;
        for (int i = 0; i < num; i++)
        {
    ...
    double GetDistance(Point fuck) //点到凸包上的最近距离（枚举每个面到这个点的距离）
    {
        double min = inf;
        for (int i = 0; i < num; i++)
        {
